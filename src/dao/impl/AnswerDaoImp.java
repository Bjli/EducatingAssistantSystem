...
public class AnswerDaoImp implements AnswerDao {
	...
	public void addAnswer(AnswerInfo answer) throws SQLException, ParseException {
		...
		if (aList.size() == 0) {
			...
			qr.update(sql, answer.getAnswerid(), answer.getUserid(), answer.getUsername(), time.format(nowTime),answer.getContent(), answer.getWorkid(), answer.getWorktitle(), answer.getWorkuser(),answer.getCourseId(), "“—Ã·Ωª",answer.getWorkuserid());
